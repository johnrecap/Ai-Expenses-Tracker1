...
class AccountProfileScreen extends StatelessWidget {
  const AccountProfileScreen({required this.user, this.profileService, super.key});
  final AppUser user;
...
class _View extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<AccountProfileCubit, AccountProfileState>(
      listenWhen: (prev, curr) =>
          (prev.messageKey != curr.messageKey && curr.messageKey != null) ||
          prev.reauthRequest != curr.reauthRequest,
      listener: (context, state) {
        ...
        if (msg != null) {
          showAppToast(
            ...
            msg,
            isError:
                state.messageKey != AccountProfileMessageKey.displayNameUpdated &&
                state.messageKey != AccountProfileMessageKey.passwordResetSent &&
                state.messageKey != AccountProfileMessageKey.emailUpdated &&
                state.messageKey != AccountProfileMessageKey.reauthSucceeded &&
                state.messageKey != AccountProfileMessageKey.accountDeleted,
          );
        }
        ...
        if (request != null && state.messageKey == AccountProfileMessageKey.reauthRequired) {
          unawaited(_showReauthDialog(context, request));
        }
      },
  ...
  String? _messageText(AccountProfileMessageKey? key) {
    ...
    return {
      ...
      AccountProfileMessageKey.emailUpdated: 'Check your new email to finish the change.',
  ...
  Future<void> _showReauthDialog(BuildContext context, ReauthRequest request) async {
    final cubit = context.read<AccountProfileCubit>();
    if (request.requiresPassword) {
      final password = await _showPasswordReauthDialog(context);
      if (!context.mounted) return;
      if (password == null) {
        cubit.cancelReauthentication();
        return;
      }
      cubit.completeReauthentication(password: password);
      return;
    }

    if (request.usesGoogle) {
      final confirmed = await showDialog<bool>(
        context: context,
        barrierDismissible: false,
        builder: (_) => AlertDialog(
          title: const Text('Verify with Google'),
          content: const Text('Sign in with Google again to continue.'),
          actions: [
      ...
      if (!context.mounted) return;
      if (confirmed == true) {
        cubit.completeReauthentication();
      } else {
        cubit.cancelReauthentication();
      }
      return;
  ...
  Future<String?> _showPasswordReauthDialog(BuildContext context) async {
...
class _IdentityCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final name = AccountIdentity.displayName(
      user: state.user,
        ...
        children: [
          Row(
            children: [
              ...
              IconButton(
                onPressed: state.isBusy || !state.capabilities.canEditLocalDisplayName
                    ? null
          ...
          _Row(
            ...
            label: 'Email',
            value: state.user.email.trim().isNotEmpty ? state.user.email.trim() : 'N/A',
          ),
          _Row(
            icon: Icons.login_outlined,
            label: 'Provider',
            value: state.capabilities.providerLabel,
          ),
          _Row(
            ...
            trailing: IconButton(
              onPressed: () {
                Clipboard.setData(ClipboardData(text: state.user.userId));
                showAppToast(context, 'ID copied');
              },
      ...
      child: Column(
        children: [
          ...
          ListTile(
            ...
            subtitle: Text(
              state.capabilities.canUpdateEmail ? 'Change your email address' : 'Not available',
              style: AppTextStyles.bodySmall.copyWith(color: AppColors.onSurfaceVariant),
            ),
